...
const FAQPage = () => {
  ...
  return (
    <div className="relative min-h-screen overflow-x-hidden bg-gradient-to-br from-amber-50 to-yellow-50">
      ...
      <div ref={faqSectionRef} className="mx-auto max-w-3xl px-4 py-16">
        <h2 className="mb-10 text-center text-3xl font-extrabold text-yellow-700 md:text-4xl">Frequently Asked Questions</h2>
        <div className="space-y-6">
          {faqs.map((faq) => (
            <details
              key={faq.q}
              className="group rounded-2xl border border-yellow-200 bg-white/95 p-7 shadow-xl transition-all duration-300 hover:shadow-amber-200/50"
            >
              <summary className="flex cursor-pointer items-center justify-between text-xl font-semibold text-yellow-700 outline-none transition-colors duration-200 group-open:text-yellow-900">
                <span>{faq.q}</span>
              ...
              <div className="group-open:animate-fade-in mt-4 text-base text-gray-700 transition-all duration-300">
                {faq.a}
              </div>
            </details>
          ))}
